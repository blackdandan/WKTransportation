...
import com.wk.wktransportation.util.ThreadFactoryBuilder;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = App.getApplication().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " "+App.getApplication().getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " "+App.getApplication().getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " "+App.getApplication().getString(R.string.str_printer_error);
                                }
                                System.out.println(App.getApplication().getString(R.string.str_state) + status);
                                Utils.toast(App.getApplication(), status);
                            }
                        }
